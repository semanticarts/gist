...
- Each major release project includes an issue documenting the updates required to delete the deprecated term in the major release; create this issue if it does not already exist. Each item should be included in a separate comment so it is possible to link to it. See issue [#947](https://github.com/semanticarts/gist/issues/947) for release `13.0.0` as an example.
